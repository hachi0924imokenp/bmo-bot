exports.run = async (client, message, args) => {
    if(!message.member.roles.cache.some(r=>["🌟 Modo T'chat  🌟", "👑 Fondateurs 👑", "👑 Fondateur Principal 👑"].includes(r.name)))
        return message.channel.send(`Désolé @${message.author.tag}, vous n'avez pas la permission à l'utilistion nécessaire de cette commande.`);
    ...
    let reason = args.slice(1).join(' ');
        if(!reason) reason = "Vous avez commis une infraction, un modérateurs vous a donc bannis";
