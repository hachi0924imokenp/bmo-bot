module.exports = (globalVariables) => {
  ...
  async function command(message, args, msg, title, options, timeout = 30, emojiList = defEmojiList.slice(), forceEndPollEmoji = '\u2705'){
    ...
    class Poll {
      ...
      async finish(client) {
        ...
        embed.setColor("FF0800").setAuthor(`${this.question} [FINISHED]`).setFooter(`Poll ${this.id} finished ${now.toUTCString()}`);
        try {
          await message.edit({
            embed: embed
          });
      ...
      async showResults(channel) {
        ...
        return await channel.send({
          embed: this.generateResultsEmbed()
        });
      }
      generateEmbed() {
        let str = new String();
        ...
        let embed = new Discord.RichEmbed().setColor("#50C878").setAuthor("📊" + this.question).setDescription(str).setFooter(footer);
        return embed;
      }
      generateResultsEmbed() {
        ...
        let resultsEmbed = new Discord.RichEmbed().setAuthor("Results of: " + this.question).setDescription(description).setFooter(footer).setColor("#0080FF");
        return resultsEmbed;
      }
      generateId() {
        let id = new String("");
