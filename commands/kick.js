module.exports = (globalVariables) => {
  ...
  async function command(message, args){
    ...
    if(member.id === client.user.id)  
      return message.channel.send("Hahaha, bien essayer mais je ne peux pas me kické moi même !");
    
    if (member.user.bot) 
      return message.channel.send("Impossible de kické un bot !");
    
    if(member.id === message.author.id) 
      return message.channel.send("Vous ne pouvez pas vous kické vous-même");
